"""
QRL (Quasimetric Reinforcement Learning) — OGBench canonical implementation

Uses the QRLAgent from the OGBench impls directory (JAX/Flax), following the
same invocation pattern as main_gcivl.py for GCIVLAgent.  All --flag
configurations and result-saving logic match main_qrl.py exactly.

OGBENCH_IMPLS must point to the ogbench/impls directory that contains:
    agents/qrl.py        – QRLAgent class
...
from agents.qrl import QRLAgent, get_config as get_agent_config
...
def build_agent_config(
    ...
    """Map config_qrl EnvConfig → ml_collections.ConfigDict for QRLAgent.

    Fields present in config_qrl are written explicitly; all other fields
    retain their OGBench defaults from get_agent_config().
    """
    agent_cfg = get_agent_config()
    agent_cfg.unlock()

    # ── hyper-parameters from config_qrl ──────────────────────────────────────
    agent_cfg.lr          = cfg.lr
    agent_cfg.batch_size  = cfg.batch_size
    agent_cfg.discount    = cfg.discount
    agent_cfg.alpha       = cfg.alpha
    agent_cfg.lam_eps     = cfg.lam_eps
...
def _qrl_c_seed_worker(kwargs: dict):
    """Run one seed of QRL (OGBench canonical) training in a subprocess.
...
def main():
    ...
    if args.parallel_seeds and len(seeds) > 1:
        ...
        with ctx.Pool(processes=len(seeds)) as pool:
            results = pool.map(_qrl_c_seed_worker, worker_args)
    ...
    # ── 4. Print report ───────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"FINAL RESULTS -- QRL (OGBench) -- {env_name}")
    ...
    out_path  = os.path.join(
        args.output_dir,
        f"results_qrl_c_{args.env}_{args.dsize}_{args.task}_{timestamp}.txt",
    )
    with open(out_path, "w") as f:
        f.write(f"method: QRL (OGBench canonical)\n")
        ...
        f.write(f"alpha={agent_cfg.alpha}  lam_eps={agent_cfg.lam_eps}  "
                f"discount={agent_cfg.discount}\n")
