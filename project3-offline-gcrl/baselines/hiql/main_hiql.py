"""
HIQL (Hierarchical Implicit Q-Learning) — OGBench canonical implementation

Uses the HIQLAgent from the OGBench impls directory (JAX/Flax), following the
same invocation pattern as main_gcivl.py for GCIVLAgent.  All --flag
configurations and result-saving logic match main_hiql.py exactly.

OGBENCH_IMPLS must point to the ogbench/impls directory that contains:
    agents/hiql.py       – HIQLAgent class
    utils/datasets.py    – Dataset, HGCDataset
...
from agents.hiql import HIQLAgent, get_config as get_agent_config
...
def build_agent_config(
    ...
    """Map config_hiql EnvConfig → ml_collections.ConfigDict for HIQLAgent.

    Fields present in config_hiql are written explicitly; all other fields
    (tau, expectile, layer_norm, const_std, low_actor_rep_grad, gc_negative,
    p_aug, frame_stack, actor_hidden_dims, value_hidden_dims) retain their
    OGBench defaults from get_agent_config().
    """
    agent_cfg = get_agent_config()
    agent_cfg.unlock()

    # ── hyper-parameters from config_hiql ─────────────────────────────────────
    agent_cfg.lr                 = cfg.lr
    agent_cfg.batch_size         = cfg.batch_size
    agent_cfg.discount           = cfg.discount
    agent_cfg.high_alpha         = cfg.high_alpha
    agent_cfg.low_alpha          = cfg.low_alpha
    agent_cfg.subgoal_steps      = cfg.subgoal_steps
    agent_cfg.rep_dim            = cfg.rep_dim
    ...
    # actor_p_curgoal not in config_hiql; OGBench default is 0.0
...
def _hiql_c_seed_worker(kwargs: dict):
    """Run one seed of HIQL (OGBench canonical) training in a subprocess.
    ...
    train_dataset = HGCDataset(Dataset(train_raw), agent_cfg)
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"HIQL (OGBench canonical) on OGBench: {env_name}")
    print(f"  Seeds          : {seeds}"
    ...
    print(f"  Batch size     : {agent_cfg.batch_size}  |  LR={agent_cfg.lr}  "
          f"gamma={agent_cfg.discount}  tau={agent_cfg.tau}  "
          f"expectile={agent_cfg.expectile}")
    print(f"  alpha_hi={agent_cfg.high_alpha}  alpha_lo={agent_cfg.low_alpha}  "
          f"k={agent_cfg.subgoal_steps}  rep_dim={agent_cfg.rep_dim}")
    print(f"  Value goals    : p_cur={agent_cfg.value_p_curgoal}  "
          f"p_traj={agent_cfg.value_p_trajgoal}  p_rand={agent_cfg.value_p_randomgoal}")
    ...
    if args.parallel_seeds and len(seeds) > 1:
        ...
        with ctx.Pool(processes=len(seeds)) as pool:
            results = pool.map(_hiql_c_seed_worker, worker_args)
    ...
    # ── 4. Print report ───────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"FINAL RESULTS -- HIQL (OGBench) -- {env_name}")
    ...
    out_path  = os.path.join(
        args.output_dir,
        f"results_hiql_c_{args.env}_{args.dsize}_{args.task}_{timestamp}.txt",
    )
    with open(out_path, "w") as f:
        f.write(f"method: HIQL (OGBench canonical)\n")
        ...
        f.write(f"discount={agent_cfg.discount}  high_alpha={agent_cfg.high_alpha}  "
                f"low_alpha={agent_cfg.low_alpha}  "
                f"subgoal_steps={agent_cfg.subgoal_steps}\n")
        f.write(f"tau={agent_cfg.tau}  expectile={agent_cfg.expectile}\n")
