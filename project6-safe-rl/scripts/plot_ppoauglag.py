"""
plot_ppoauglag.py — Figure 7-style plots for PPOAugLag results.

Direct mirror of plot_ppolag_trpolag.py, adapted for PPOAugLag's OmniSafe output.
...
Usage
-----
# Auto-locate latest PPOAugLag run under runs/:
    python scripts/plot_ppoauglag.py

# Specify a CSV directly:
    python scripts/plot_ppoauglag.py \\
        --csv "runs/PPOAugLag-{SafetyPointGoal2-v0}/seed-000-.../progress.csv"

# Override algorithm prefix for auto-discovery:
    python scripts/plot_ppoauglag.py --algo PPOAugLag

# Override output file name:
    python scripts/plot_ppoauglag.py --out figs/ppoauglag_pointgoal.png
...
ALGO_LABEL    = 'PPOAugLag'
COLOR         = '#17becf'              # cyan — distinct from blue/red/orange/green/purple


def find_latest_progress(algo_prefix: str = 'PPOAugLag') -> Path:
    paths = sorted(Path('runs').glob(f'{algo_prefix}-*/*/progress.csv'))
    if not paths:
        raise FileNotFoundError(
            f"No progress.csv found under runs/{algo_prefix}-*/ — "
            "run run_ppoauglag.py first, or pass --csv explicitly."
        )
    return paths[-1]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Plot PPOAugLag training results (Figure 7 style).'
    )
    parser.add_argument(
        '--algo', type=str, default='PPOAugLag',
    ...
    for p in csv_path.parts:
        if 'PPOAugLag-' in p:
            env_tag = p.replace('PPOAugLag-', '').strip('{}')
            break
