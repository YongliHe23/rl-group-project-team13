"""
plot_ppolagfuzrl.py — Figure 7-style plots for PPOLagFuzRL results.

Direct mirror of plot_ppoauglag.py, adapted for PPOLagFuzRL's OmniSafe output.
...
Plots three panels matching the OpenAI Safe Exploration evaluation criteria:
  ...
  2. AverageEpCost  — episodic constraint cost (safety, with cost_limit line at 25)
  3. CostRate       — cumulative cost / total steps (safety regret, ρ_c = Σc_t / T)

Usage
-----
# Auto-locate latest PPOLagFuzRL run under runs/:
    python scripts/plot_ppolagfuzrl.py

# Specify a CSV directly:
    python scripts/plot_ppolagfuzrl.py \\
        --csv "runs/PPOLagFuzRL-{SafetyPointGoal2-v0}/seed-000-.../progress.csv"

# Override algorithm prefix for auto-discovery:
    python scripts/plot_ppolagfuzrl.py --algo PPOLagFuzRL

# Override output file name:
    python scripts/plot_ppolagfuzrl.py --out figs/ppolagfuzrl_pointgoal.png
...
ALGO_LABEL    = 'PPOLagFuzRL'
COLOR         = '#e377c2'              # magenta/pink — distinct from all existing algorithm colors


def find_latest_progress(algo_prefix: str = 'PPOLagFuzRL') -> Path:
    paths = sorted(Path('runs').glob(f'{algo_prefix}-*/*/progress.csv'))
    if not paths:
        raise FileNotFoundError(
            f"No progress.csv found under runs/{algo_prefix}-*/ — "
            "run run_ppolagfuzrl.py first, or pass --csv explicitly."
        )
    return paths[-1]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Plot PPOLagFuzRL training results (Figure 7 style).'
    )
    parser.add_argument(
        '--algo', type=str, default='PPOLagFuzRL',
    ...
    for p in csv_path.parts:
        if 'PPOLagFuzRL-' in p:
            env_tag = p.replace('PPOLagFuzRL-', '').strip('{}')
            break
