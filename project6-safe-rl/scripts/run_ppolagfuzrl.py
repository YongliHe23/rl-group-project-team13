"""
run_ppolagfuzrl.py — Grid runner for PPOLagFuzRL (PPO-Lag + Advanced Cost-GAE + Fuz-RL).

Mirrors run_ppoauglag.py: calls create_agent() directly for each (env_id, seed)
...
forwarded correctly.  OmniSafe writes its own output directory:
    ...
    runs/PPOLagFuzRL-{env_id}/seed-{seed:03d}-{timestamp}/progress.csv

Usage examples
--------------
# Single env, single seed (quick test):
    python scripts/run_ppolagfuzrl.py \\
...
def _worker(job: dict) -> dict:
    """Run a single (env_id, seed) PPOLagFuzRL experiment and return a summary."""
    import sys
    project_root = job['project_root']
    if project_root not in sys.path:
        sys.path.insert(0, project_root)

    from scripts.train_ppolagfuzrl import create_agent
    ...
    print(f"\n[PPOLagFuzRL grid] env={env_id}  seed={seed}", flush=True)
    t0 = time.time()
    ...
    print(
        f"[PPOLagFuzRL grid] DONE  env={env_id}  seed={seed}  "
...
def run_grid(
    ...
    n = len(jobs)
    print(f"\n{'='*60}")
    print(f"  PPOLagFuzRL (PPO-Lag + Advanced Cost-GAE + Fuz-RL) Grid")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Run PPOLagFuzRL over an experiment grid of environments and seeds.'
    )
    parser.add_argument(
        '--envs', nargs='+',
    ...
    parser.add_argument(
        '--config',
        default='configs/ppolagfuzrl/config.yaml',
...
# Examples:
#   python scripts/run_ppolagfuzrl.py --envs SafetyPointGoal2-v0 --seeds 0
#   python scripts/run_ppolagfuzrl.py --envs SafetyPointGoal2-v0 --seeds 0 1 2 --num_pool 3
